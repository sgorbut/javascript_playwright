...
test('Second test', async ({page}) =>
{
    ...
    await page.getByLabel('Search or jump to…').click();    // click on search field
    await page.getByRole('combobox', { name: 'Search' }).fill('playwright');    // enter text
    await page.getByRole('combobox', { name: 'Search' }).press('Enter');    // press Enter
    ...
    await page.getByLabel('Search or jump to…').click();    // click on search field
    await page.getByRole('combobox', { name: 'Search' }).fill('appium');    // enter text
    await page.getByRole('combobox', { name: 'Search' }).press('Enter');    // press Enter
    // console.log(await page.textContent('.d-flex > h3'));    // print text
